...
On startup, queries the pool (GET /pool) to resolve a browser ID, then
sends all commands to that specific browser via /browsers/{id}/command.
This ensures analyze → click → type all hit the same browser within a
thread, even though each skill invocation is a fresh process. The pool
list order is stable (Map insertion order), so browsers[0] is consistent
across invocations.

The REST API uses `selector` (CSS) for element targeting, not `element_id`.
The analyzer writes data-ac-id="N" attributes onto DOM elements, so we
translate element_id → [data-ac-id="N"].
...
def resolve_browser(client: httpx.Client) -> str:
    """Pick a browser from the pool. Returns the first browser's ID.

    The pool list order is stable (server uses Map insertion order),
    so this consistently returns the same browser across invocations.
    """
    r = client.get(
        f"{API_BASE}/pool",
...
def cmd(client: httpx.Client, bid: str, action: str, params: dict | None = None, timeout: int = 35) -> dict:
    """Send a command to a specific browser and return the parsed response.
...
try:
    ...
    with httpx.Client(timeout=60) as client:
        # Resolve a browser from the pool once — all commands in this
        # invocation (and future invocations) hit the same browser because
        # the pool list order is stable (server Map insertion order).
        bid = resolve_browser(client)
        if not bid:
            print(json.dumps({"error": "No browsers connected — open the browser extension and connect"}))
            raise SystemExit(0)
